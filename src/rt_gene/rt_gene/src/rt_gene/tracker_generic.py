...
class TrackedSubject(object):
    ...
    def get_eye_image_from_landmarks(subject, eye_image_size):
        ...
        try:
            ...
            aligned_face, rot_matrix = GenericTracker.align_face_to_eyes(
                subject.face_color,
            ...
            left_bb[1] = (
                lefteye_center_y - (lefteye_width + lefteye_margin) * desired_ratio
            )
            ...
            left_eye_color_resized = cv2.resize(
                left_eye_color, eye_image_size, interpolation=cv2.INTER_CUBIC
            )
            right_eye_color_resized = cv2.resize(
                right_eye_color, eye_image_size, interpolation=cv2.INTER_CUBIC
            )
...
class GenericTracker(object):
    ...
    def get_distance_matrix(self, new_elements):
        ...
        distance_matrix = np.full(
            (len(self._tracked_elements), len(new_elements)), np.inf
        )
        for i, element_id in enumerate(self._tracked_elements.keys()):
            ...
            for j, new_element in enumerate(new_elements):
                ...
                if not isinstance(new_element, TrackedSubject):
                    raise TypeError(
                        "Inappropriate type: {} for element whereas a TrackedSubject is expected".format(
                            type(new_element)
                        )
                    )
                distance_matrix[i][j] = self._tracked_elements[
                    element_id
                ].compute_distance(new_element)
        return distance_matrix, map_index_to_id

    @staticmethod
    def align_face_to_eyes(
        face_img, right_eye_center, left_eye_center, face_width=None, face_height=None
    ):
        ...
        desired_face_height = (
            face_height if face_height is not None else face_img.shape[0]
        )
    ...
    def update_eye_images(self, eye_image_size):
        for subject in self.get_tracked_elements().values():
            le_c, re_c, le_bb, re_bb = TrackedSubject.get_eye_image_from_landmarks(
                subject, eye_image_size
            )
