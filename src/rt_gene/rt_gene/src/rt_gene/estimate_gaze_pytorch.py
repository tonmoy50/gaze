...
from gaze_estimation_models_pytorch import (
    GazeEstimationModelVGG,
    GazeEstimationModelResnet18,
)
...
class GazeEstimator(GazeEstimatorBase):
    def __init__(
        ...
        self._transform = transforms.Compose(
            [
                lambda x: cv2.resize(x, dsize=(60, 36), interpolation=cv2.INTER_CUBIC),
                transforms.ToTensor(),
    ...
    def estimate_gaze_twoeyes(
        ...
        transformed_left = torch.stack(inference_input_left_list).to(
            self.device_id_gazeestimation
        )
        transformed_right = torch.stack(inference_input_right_list).to(
            self.device_id_gazeestimation
        )
        tranformed_head = torch.as_tensor(inference_headpose_list).to(
            self.device_id_gazeestimation
        )

        result = [
            model(transformed_left, transformed_right, tranformed_head).detach().cpu()
