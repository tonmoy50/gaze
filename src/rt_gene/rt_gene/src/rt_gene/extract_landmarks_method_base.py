...
facial_landmark_transform = transforms.Compose(
...
class LandmarkMethodBase(object):
    def __init__(
        ...
        tqdm.write(
            "Using device {} for face detection.".format(device_id_facedetection)
        )

        self.device = device_id_facedetection
        self.face_net = SFDDetector(
            device=device_id_facedetection, path_to_detector=checkpoint_path_face
        )
    ...
    def load_face_landmark_model(self, checkpoint_fp=None):
        import ThreeDDFA.mobilenet_v1 as mobilenet_v1

        if checkpoint_fp is None:
            import rospkg

            checkpoint_fp = (
                rospkg.RosPack().get_path("rt_gene")
    ...
    def get_full_model_points(self, model_points_file=None):
        ...
        if model_points_file is None:
            ...
            model_points_file = (
                rospkg.RosPack().get_path("rt_gene") + "/model_nets/face_model_68.txt"
            )
        ...
        model_points = model_points * (
            self.interpupillary_distance * self.model_size_rescale
        )
    ...
    def ddfa_forward_pass(self, color_img, roi_box_list):
        ...
        _input = torch.cat(
            [facial_landmark_transform(img).unsqueeze(0) for img in img_step], 0
        )
        with torch.no_grad():
