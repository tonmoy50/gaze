...
class GFIETestDataset(Dataset):
    ...
    def __init__(self, opt):
        ...
        transform_list.append(
            transforms.Resize((opt.TRAIN.input_size, opt.TRAIN.input_size))
        )
    ...
    def __getitem__(self, index):
        ...
        # format input
        format_input = self.format_model_input(
            rgb_path, depth_path, head_bbox, self.camerapara, eye_3d
        )
    ...
    def format_model_input(self, rgb_path, depth_path, head_bbox, campara, eye_coord):
        ...
        x_min, y_min, x_max, y_max = map(
            float, img_utils.expand_head_box(head_bbox, [width, height])
        )
        ...
        head_loc = img_utils.get_head_box_channel(
            x_min,
            y_min,
            x_max,
            y_max,
...
class CAD120TestDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        rgb_path = os.path.join(
            self.rgb_path, "{}".format(scene_id), "RGB_{}.png".format(frame_index)
        )
        depth_path = os.path.join(
            self.depth_path, "{}".format(scene_id), "Depth_{}.png".format(frame_index)
        )
        ...
        # format input
        format_input = self.format_model_input(
            rgb_path, depth_path, head_bbox, self.camerapara, eye_3d, index
        )
    ...
    def format_model_input(
        self, rgb_path, depth_path, head_bbox, campara, eye_coord, index=None
    ):
