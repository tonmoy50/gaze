    ...
    def __getitem__(self, index):
        ...
        if self.dstype == "train":
            ...
            if np.random.random_sample() <= 0.5:
                ...
                # crop depth img
                depthimg = TF.crop(
                    depthimg, crop_y_min, crop_x_min, crop_height, crop_width
                )
            ...
            gaze_u, gaze_v = gaze_u / float(width), gaze_v / float(height)

            eye_u, eye_v = eye_u / float(width), eye_v / float(height)

        # represent the head location with mask
        head_channel = img_utils.get_head_box_channel(
            x_min,
            y_min,
            x_max,
            y_max,
        ...
        scale_width, scale_height = (
            final_width / self.input_size,
            final_height / self.input_size,
        )
        ...
        if flip_flag:
            cx = org_width - cx
            matrix_T_X = (
                (matrix_T_xx * scale_width + (org_width - final_width - offset_x) - cx)
                * depthmap
                / fx
            )
        ...
        gaze_heatmap = img_utils.draw_labelmap(
            gaze_heatmap,
            [gaze_u * self.output_size, gaze_v * self.output_size],
            3,
            type="Gaussian",
        )
        ...
        all_data["gaze_target3d"] = torch.from_numpy(
            np.array([gaze_X, gaze_Y, gaze_Z]) - np.array([eye_X, eye_Y, eye_Z])
        )
